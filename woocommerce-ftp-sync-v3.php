<?php
/*
Plugin Name: WooCommerce FTP Sync San Pablo
Description: Sincronización SAP avanzada de productos FTP a WooCommerce.
Version: 1.4
Author: Ivan Bello
Author URI: http://www.lunarpolis.online
*/
...
class WooCommerce_FTP_Sync {
    ...
    public function __construct() {
        ...
        add_action('woocommerce_product_options_general_product_data', [$this, 'add_sync_checkbox']);
        add_action('woocommerce_process_product_meta', [$this, 'save_sync_checkbox']);
    ...
    public function sync_products() {
        // Verificar que el archivo CSV se descargue correctamente
        ...
        foreach ($csv_data as $index => $row) {
            ...
            // Establecer el precio rebajado vacío si no hay descuento
            $sale_price = $discount > 0 ? $price - ($price * ($discount / 100)) : '';  // Dejar vacío si no hay descuento
            ...
            if ($product_id) {
                // Verificar si el producto tiene activada la sincronización FTP
                $sync_ftp = get_post_meta($product_id, '_sync_ftp', true);
                // Asegurarse de que sólo se sincroniza si está marcado
                if ($sync_ftp === 'yes') {  // Verificar que el valor es 'yes'
                    wp_update_post(['ID' => $product_id, 'post_title' => sanitize_text_field($data['name'])]);

                    // Actualizar precios
                    update_post_meta($product_id, '_regular_price', $price); // Actualizar precio regular
                    update_post_meta($product_id, '_price', $sale_price ?: $price); // Si el precio rebajado está vacío, se usa el precio regular
                    update_post_meta($product_id, '_sale_price', $sale_price); // Actualizar precio rebajado

                    // Actualizar stock y otros datos
                    ...
                    // Forzar actualización del producto (esto también asegura que el stock se guarde correctamente)
                    $product = wc_get_product($product_id); // Obtener objeto producto
                    $product->set_stock_quantity($stock); // Aseguramos que el stock se guarde correctamente
                    $product->set_manage_stock(true); // Habilitar la gestión de stock si no está habilitada
    ...
    // Agregar checkbox para habilitar/deshabilitar la sincronización FTP
    public function add_sync_checkbox() {
        global $post;
        // Establecer el valor del checkbox a "yes" por defecto si no está guardado
        $sync_ftp_value = get_post_meta($post->ID, '_sync_ftp', true);
        // Si no hay valor, se asume que debe estar marcado por defecto
        $checked = empty($sync_ftp_value) || $sync_ftp_value === 'yes' ? 'yes' : 'no';

        woocommerce_wp_checkbox([
            'id' => '_sync_ftp',
            'label' => 'Sincronizar FTP',
            'description' => 'Habilitar esta opción para sincronizar este producto con el FTP.',
            // Marcar por defecto
            'value' => $checked,
        ]);
    }
    ...
    public function save_sync_checkbox($post_id) {
        $sync_ftp = isset($_POST['_sync_ftp']) ? 'yes' : 'no';
        update_post_meta($post_id, '_sync_ftp', $sync_ftp);
    }
}
